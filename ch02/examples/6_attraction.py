...
def main() -> None:
    ...
    mover.velocity = pygame.Vector2(2, 0)
    ...
    while True:
        for event in pygame.event.get():
            ...
            mosue_position = pygame.Vector2(*pygame.mouse.get_pos())
            if event.type == pygame.MOUSEBUTTONDOWN:
                attractor.handle_press(mosue_position)
            if event.type == pygame.MOUSEBUTTONUP:
                attractor.stop_dragging()
            if event.type == pygame.MOUSEMOTION:
                attractor.handle_drag(mosue_position)
                attractor.handle_hover(mosue_position)
